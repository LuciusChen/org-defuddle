* pulldown-cmark footnote-definition
:PROPERTIES:
:WORD_COUNT: 110
:END:
...
Example bit-shift analysis: a common simplification is to replace ~x & \~0~ with ~x~ [fn:1]. Other paragraph content follows that explains the broader idea in more detail.
...
[fn:1] Possibly with masking of the top bit if our IR semantics have defined wrapping/truncation behavior: ~x & 0x7fff..ffff~

...
:PROPERTIES:
:URL: https://12gramsofcarbon.com/p/ilyas-30-papers-to-carmack-vlaes
:AUTHOR: theahura
:PUBLISHED: 2025-03-06T12:48:40+00:00
:SITE: 12 Grams of Carbon
:DOMAIN: 12gramsofcarbon.com
:DESCRIPTION: I'm increasingly certain no one knows anything about anything. Previous founder @ SOOT. Before that was googling. Writes about programming, AI, startups, video games, and misc other things.
:IMAGE: https://substackcdn.com/image/fetch/$s_!Hs9y!,w_1200,h_675,c_fill,f_jpg,q_auto:good,fl_progressive:steep,g_auto/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fffb5afd7-ffc8-402d-8eaa-4060938fe1d2_1035x449.png
:LANGUAGE: en
:WORD_COUNT: 4029
:END:

This post is part of a series of paper reviews, covering the ~30 papers Ilya Sutskever sent to John Carmack to learn about AI. To see the rest of the reviews, go [[https://theahura.substack.com/p/ilyas-30-papers-to-carmack-table][here]].
...
The author's propose a pretty straightforward solution, but I want to go on a tangent first. To really understand what the authors are doing, we need a better understanding of what a VAE actually is, and how this all relates to Maximum LIkelihood Estimation (MLE). If that's not interesting to you, feel free to jump a few paragraphs (cntrl-f "back to the paper"). [fn:2]

Let's say we have a scatter plot with data that is sampled from some distribution, /*X = { (x 1, y 1), (x 2, y 2)... }*/. We want to learn a generative model that tries to predict the original distribution the data was sampled from. Note how this differs from standard classification. We are not trying to figure out /*y*/ given /*x*/ or anything like that. We are trying to directly model the real original data source, with all of the variance between /*x*/ and /*y*/ baked in.

We can represent our model with a set of parameters, /*θ*/. We want to find /*θ*/ that best 'explains' the data that we see. In other words, we want /*p( X | θ )*/ to be high. This is called 'maximum likelihood estimation' — we are finding the model that has the "maximum likelihood" of producing the observations we see. [fn:3]
...
It gets worse. Even if you manage to solve the training optimization issues, the authors show that it's always optimal for the RNN to ignore the latent variable. This goes all the way back to the [[https://theahura.substack.com/p/ilyas-30-papers-to-carmack-nn-regularization][Minimum Description Length (MDL) paper]]. Quoting from that review:
...
Because IAFs are more expressive than a standard gaussian output, the theory is that /*z*/ has less of an impact on the KL divergence regularization term, which in turn results in the latent being used more. [fn:5]
...
One thread is what I've been calling the "complexity" thread. This thread is all about model complexity — how can we measure it, how does it behave, when do we want more or less complex models, things like that. And we've already seen a few papers in this thread. The [[https://theahura.substack.com/p/ilyas-30-papers-to-carmack-nn-regularization][MDL paper]] was the first chronologically, which laid out the idea that model complexity is something we actually should care about minimizing. And the [[https://theahura.substack.com/p/ilyas-30-papers-complextropy][complextropy paper]] followed up by trying to define a measure of complexity that we can actually reason about. [fn:6]
...
All that said, looking back from 2025, I think we've mostly as a field moved away from using probabilistic latents in popular benchmark-breaking models. There are some areas of research like the [[https://arxiv.org/abs/2206.00826][Bayesformer]], but most representation models these days use some form of semi-supervised loss like CLIP (contrastive loss) or BERT (self-token-prediction). These methods scale much better with massive data repositories, which seems to be more useful than inlining smoothing functions using embedded random variables.

[fn:1] Note that there is a bit of a blurred line between learning conditions and learning joint distributions. You can train a classifier to learn conditions, but then simply take an intermediate embedding representation from that classifier and use that to generate new data. The intermediate representation may not be very good, but it is possible, and that makes this whole conversation a bit fuzzy — like everything else in deep learning.

[fn:2] In deep models, MLE provides a theoretical grounding for figuring out your loss function. Cross entropy loss or MSE are instantiations of maximum likelihood estimation given certain priors, like categorical data or a gaussian distribution, respectively. That said, in practice I think you can mostly ignore MLE and just short-hand it to ‘cross entropy loss for categories, error losses for regression’.

[fn:3] This was always hard for me to understand because it felt backwards! I always thought you would want to find the parameters conditioned on the data points. But the data comes out of the model, not the other way around.

[fn:4] That's not to say that MDL is wrong. MDL refers to the complexity of the model and the number of bits necessary to communicate the data through the model. Latent variables introduce a form of a loss term on the model complexity but don't capture the total complexity. The theory behind a RNN + VAE is that the combination may be a more complex model, but should significantly reduce the total complexity by decreasing how difficult it is to represent data.

[fn:5] I'm not fully clear on why they needed this to be an IAF instead of a standard mlp. The IAF does have some additional structure, so maybe that helps in some way?

[fn:6] Spoiler: the coffee automaton paper that is coming up in Paper 20 is also in this thread.
